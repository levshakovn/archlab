...
function ArchitectureVisualization() {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      setServices(prev => prev.map(service => {
        ...
        let newX = service.x + moveX
        let newY = service.y + moveY
  ...
  return (
    <div className="bg-white rounded-xl shadow-lg p-8 overflow-hidden">
      ...
      <div 
        ref={containerRef}
        className="relative h-96 bg-gradient-to-br from-blue-50 to-teal-50 rounded-lg overflow-hidden"
      >
        {/* SVG for connections */}
        ...
        {services.map((service) => (
          <div
            key={service.id}
            className="absolute bg-white border-2 border-primary rounded-lg shadow-md p-3"
            style={{
              left: `${service.x}px`,
              top: `${service.y}px`,
              width: '120px',
              zIndex: 2,
              transition: 'none', // We handle animation manually for smoother control
            }}
          >
    ...
    <div className="flex flex-col items-center justify-center gap-1">
      {!logoError && (
        <img
          ...
          className="w-10 h-10 object-contain flex-shrink-0 transition-opacity"
