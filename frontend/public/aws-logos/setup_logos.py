    ...
    for svg_file in icons_dir.rglob("*.svg"):
        ...
        if parent_name.isdigit():
            ...
            if service_name not in svg_files:
                ...
                if size == preferred_size or (existing_size != preferred_size and size > existing_size):
                    svg_files[service_name] = (svg_file, size)
...
def setup_logos(source_path: str, target_dir: str):
    ...
    if not icons_dir or not icons_dir.exists():
        print(f"❌ Error: Could not find Architecture-Service-Icons directory in {source_path}")
        print(f"   Looking for folder containing 'Architecture-Service-Icons' in the name")
        return False
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        if os.path.exists(default_path):
            ...
            print("📥 Please provide the path to the extracted AWS Architecture Icons folder")
            print()
            print("Download from: https://aws.amazon.com/architecture/icons/")
            print()
            source_path = input("Enter path to extracted icons folder (or press Enter to exit): ").strip()
